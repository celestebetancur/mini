...
C_MODULES=
CXX_MODULES=mini.cpp parser.tab.cpp lex.yy.cpp eval.cpp parser_state.cpp
...
ifeq ($(CK_CHUGIN_STATIC),0)
	...
	@rm -f $(C_OBJECTS) $(CXX_OBJECTS) parser.tab.cpp parser.tab.h parser.tab.hpp lex.yy.cpp
	@echo "[mini] built: $@"
...
# bison/flex generation rules
parser.tab.cpp parser.tab.h: parser.y
	bison -d parser.y
	@rm -f parser.tab.hpp
	mv parser.tab.c parser.tab.cpp || true

lex.yy.cpp: scanner.l parser.tab.h
	flex -o lex.yy.cpp scanner.l

# standalone test binary (uses main.cpp, not part of chugin build)
standalone: parser.tab.cpp lex.yy.cpp
	g++ -std=c++17 -Wall -g parser.tab.cpp lex.yy.cpp main.cpp eval.cpp parser_state.cpp -o parser

clean:
	rm -rf $(C_OBJECTS) $(CXX_OBJECTS) $(CHUG) $(WEBCHUG) Release Debug \
		parser.tab.cpp parser.tab.h parser.tab.hpp lex.yy.cpp parser
